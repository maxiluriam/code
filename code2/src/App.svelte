...
Storage.prototype.getObj = function (key:string) {
  return JSON.parse(this.getItem(key));
	...
	<button on:click={moveVideoLeft}  class="movebackward" type="click"> MOVE VIDEO   <p> <i class="arrow left"></i></p></button>
	<button on:click={left} type="click" class="back"> <p> <i class="arrow left"></i></p> </button>
		<iframe   src={videoList[currentVideo]} title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
	<button on:click={right} type="click" class="forward"> <p> <i class="arrow right"></i></p> </button>
	<button on:click={moveVideoRight} class="moveforward" type="click"> MOVE VIDEO <p> <i class="arrow right"></i></p></button>
